...
##args <- "" # !!! TESTING !!!
if (length(args)==0) {
  cat("Usage: teacherReport_batch.R user\n")
  cat("where\n")
  cat("* user: folder contaning users' folders\n")
  cat("Example: teacherReport_batch.R /tmp/otulea/users\n")
} else {
  ## REQUIREMENTS
  require(XML)
  require(tools)
  ...
  users.length <- length(users)
  u <- 17
  for (u in seq_along(users)[17:users.length]) {
    user <- users[u]
    cat("Iteration: ",u,"/",users.length,", user number: ",user,"\n")
    ...
    layer3
    ## breaking category if necessary
    Y.names <- names(Y)
    H <- sapply(Y.names,function(y) sapply(X,function(x) length(layer3[[x]][[y]])))
    H.list <- as.list(as.data.frame(H))
    H.list2 <- lapply(H.list,function(h) {
      a <- lapply(h, function(x) {
        k <- x%/%hmax
        m <- x%%hmax
        if (k > 0) {
          eleje <- lapply(1:k,function(i) ((i-1)*hmax+1):(i*hmax))
        } else {
          eleje <- list()
        }
        if (length(eleje)>0) {
      ...
      a.maxlength <- max(a.length)
      add <- a.maxlength-a.length
      for (i in seq_along(add)) a[[i]] <- c(a[[i]],as.list(rep(0,add[i])))
      b <- as.list(rep(0,a.maxlength))
      for (i in 1:a.maxlength) {
        bi <- lapply(seq_along(X),function(j) a[[j]][[i]])
        names(bi) <- X
        b[[i]] <- bi
      }
      b
    ...
    ##H.list
    ##length(H.list2)
    ## based on that I create a new list again
    layer4 <- list()
    layer4.cat <- c()
    ##y <- Y.names[2]
    for (y in Y.names) {
      splitn <- H.list2[[y]]
      for (spn in splitn) {
        ujelem <- sapply(X,function(x) layer3[[x]][[y]][spn[[x]]])
        layer4 <- c(layer4,list(ujelem))
        layer4.cat <- c(layer4.cat,y)
      }
    }
    layer4
    layer4.cat
    ...
    sink(file.path(tmpdir,paste("data.tex",sep="")))
    pagenum <- 0
    ##i <- 1 # !!! TESTING !!!
    for (i in seq_along(layer4)) { # looping through all pages
      catname <- layer4.cat[i]
      pagenum <- pagenum + 1
      layer4.i <- layer4[[i]]
      y <- Y[[catname]]
      ...
      cat("\\end{tikzpicture}}\n")
      cat("\\newpage\n")
    } # for (i in seq_along(layer4))
    sink()
    ## copying template file to temporary directory 
    file.copy(file.path(dir.template,"feedback_break.tex"),
