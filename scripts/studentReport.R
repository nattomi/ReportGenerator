...
graphics <- c("Check","Ladder")
graphics.files <- c("lea_blue_check.png","lea_blue_ladder.png",
...
if (nargs==0) {
  ...
  cat("Example: studentReport.R KFCG1 50 3\n")
} else {
  ## REQUIREMENTS
  #t0 <- Sys.time()
  require(XML)
  ...
  feedback2tex <- function(x,subject, mode, graphics.command) {
    if (missing(x)) {
      ...
      if (rownum > 0) {
        ...
        cat("\\begin{tabular}{r")
        cat(ifelse(beispiel,"p{.4\\textwidth}p{.4\\textwidth}","p{.8\\textwidth}"))
        cat("}\n")
        cat("& \\textcolor{TextStandard}{\\fontsize{20pt}{1em}\\selectfont ",modes.string[[mode]], "}",sep="")
        if (beispiel) cat(" & \\textcolor{TextStandard}{\\fontsize{20pt}{1em}\\selectfont Beispiel}")
        cat("\\\\[-50px]\n")
        for (i in 1:rownum) {
          cat(graphics.command,"&",x[i,"userdescription"])
          if (beispiel) cat(" & ",x[i,"example"])
          cat("\\\\\n")
        }
        cat("\\end{tabular}")
      } else {
  ...
  for (mode in modes) {
    ...
    sink(file.path(tmpdir,paste(mode,"tex",sep=".")))
    feedback2tex(tables_pro_mode[[mode]],subject,mode,
                 graphics.command)
    sink()
  }
  ## I must create a 'settings.tex' file too
  sink(file.path(tmpdir,"settings.tex"))
  cat("% subject\n")
  cat("\\toggletrue{",subject,"}\n",sep="")
  cat("% user\n")
  cat("\\def\\user{",user,"}\n",sep="")
  cat("% level\n")
  cat("\\def\\level{",tolower(level),"}\n",sep="")
  sink()
  #t3 <- Sys.time()
  ## copying template file to temporary directory 
  file.copy(file.path(dir.template,"userfeedback_dev.tex"),
