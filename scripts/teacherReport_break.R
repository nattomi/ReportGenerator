...
graphics.files <- c("arrow_grey_down.png", "logo.pdf", "arrow_grey_right.png", "arrow_grey_up.png", "blank.png")
...
if (length(args)==0) {
  ...
  cat("Example: teacherReport.R KFCG1\n")
} else {
  ## REQUIREMENTS
  require(XML)
  require(tools)
  ...
  for (i in seq_along(layer4)) { # looping through all pages
    ...
    for (x in X) {# this inner loop creates the data nodes
      ...
      for (item in names(xy)) { # looping through ability IDs
        ...
        cat("\\multicolumn{2}{l}{\\small ",tend,"\\textbf{",item,"}}\\\\\n",sep="")
